...
if (!isset($_SESSION['VISIT_SESSION_CREATED'])) {	
	...
	DB::table(FDBPrefix.'statistic_online')->insert(["","$ip",getUrl(),"$time","$browser","$platform","$country","$city","$key"]);
} 
else if (time() - $_SESSION['VISIT_SESSION_CREATED'] > 300) {
	$key  = $_SESSION['VISIT_SESSION_KEY'] = md5($ip.getUrl().$time);
    $time = $_SESSION['VISIT_SESSION_CREATED'] = time();  // update creation time
	//DB::insert(,array("","$ip",getUrl(),"$time","$browser","$platform","$country","$city","$key"));	
    DB::table(FDBPrefix.'statistic_online')->insert(["","$ip",getUrl(),"$time","$browser","$platform","$country","$city","$key"]);
...
$url = getUrl();
DB::table(FDBPrefix.'statistic_online')->where("key = '".$_SESSION['VISIT_SESSION_KEY']."'")->update(["url"=>"$url"]);
$timer = time() - 300;
...
if(!isset($_SESSION['VISIT_SESSION_DAY']) or $date != @$_SESSION['VISIT_SESSION_DAY']) {	
	...
	DB::table(FDBPrefix.'statistic')->insert(["","$ip",$_SESSION['USER_ID'],"$time","$browser","$platform","$country","$city"]);
