...
if(isset($_POST['register']) AND siteConfig('member_registration')) {
	...
	if(	!empty($_POST['password']) AND 
		...
		if(!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
			define("userNotice", alert("danger",user_Email_Invalid));
		}
			...
			if($qr) {
				...
				if($activator == 0) {
					...
					define("userNotice",alert("danger","Sorry, mail server danger :("));
				}
			}
			else 
				define("userNotice",alert("danger",user_Registration_Exists));
		} else 
			define("userNotice",alert("danger",user_Security_Invalid));
	}
	else  {						
		define("userNotice",alert("danger",user_Please_Complete_Fields));
	}
}
...
if(isset($_POST['edit'])){		
	if(!empty($_POST['email']) AND @ereg("^.+@.+\\..+$",$_POST['email'])) 
	{
		...
		if($qr AND isset($_POST['edit'])){	
			...
			define("userNotice",alert("danger",user_Password_Not_Match));
		}
		else {				
			define("userNotice",alert("danger",Status_Invalid));
		}					
	}
	else {				
		define("userNotice",alert("danger",Status_Invalid));
...
if(isset($_POST['forgot']))	{		
	if(!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)){						
		define("userNotice",alert("danger",user_Email_None));
	} 
	else {
		...
		// Mail it
			...
			if($mail) {
				...
				define("userNotice",alert("danger","System danger : function mail() can not executed."));		
		}
		else {
			define("userNotice",alert("danger",user_Email_None));
		}
...
//Set layout to follow default User Apps
if(Site::$layout == 1) {
	$id = menuInfo("layout","?app=user");
	if($id)
	Site::setLayout($id);
}
...
if($view == 'register')
	...
	define('PageTitle','Lost Password');
else if($view == 'logout') {
	define('PageTitle','User Logout');
}
