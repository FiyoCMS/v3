...
if(isset($_POST['send-comment'])){
	...
	if(empty($_POST['name']) or empty($_POST['email']) or empty($_POST['com'])) {	
		...
		if(!$name  AND $_SESSION['USER_LEVEL'] != 1 AND $_SESSION['USER_LEVEL'] != 2) {
			...
			$parent = 1;
			$apps = app_param();
			$com = DB::table(FDBPrefix.'comment')->insert(array("","$link",$_SESSION['USER_ID'],"$_POST[name]","$_POST[email]","$_POST[web]",date("Y-m-d H:i:s",time()),"$text","$auto","$apps","$parent","$parent","$parent"));
