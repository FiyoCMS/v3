  ...
  <tbody>
	<?php	
		...
		if($_SESSION['USER_LEVEL'] > 3)
			$sql = DB::Table(FDBPrefix."comment")->select("*,DATE_FORMAT(date,'%W, %b %d %Y') as dates")->orderBy("parent_user_id = $user_id OR thread_user_id = $user_id")->orderBy('date DESC LIMIT 10')->get();
		else
			$sql = DB::Table(FDBPrefix."comment")->select("*,DATE_FORMAT(date,'%W, %b %d %Y') as dates")->orderBy('date DESC LIMIT 10')->get(); 
		$no = 0;		
