...
$sql=DB::table(FDBPrefix."permalink")->where("id=$_REQUEST[id]")->get(); 
$qr = $sql[0]; 
...
<input type="hidden" name="id" value="<?php echo $qr['id']; ?>">
	<div id="app_header">
		<div class="warp_app_header">		
			...
			<div class="app_link">
				<a class="btn btn-default" href="?app=permalink" title="<?php echo Prev; ?>"><i class="icon-arrow-left"></i> <?php echo Prev; ?></a>
			
				<button type="submit" class="btn btn-success" title="<?php echo Save; ?>" value="<?php echo Save; ?>" name="apply"><i class="icon-check"></i> <?php echo Save; ?></button>	
			<?php printAlert(); ?>
			</div>			
		</div>
	</div>	
