...
header('Content-Type: application/json');
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		if ( isset($_GET['bSearchable_'.$i]) && $_GET['bSearchable_'.$i] == "true" && $_GET['sSearch_'.$i] != '' )
		{
			if ( $sWhere == "" )
			{
				$sWhere = "WHERE link LIKE '%app?=article%' ";
			}
			else
			{
				$sWhere .= " AND ";
			}
			$sWhere .= "link LsIKE '%app?sds=article%' `".$aColumns[$i]."` LIKE '%".addslashes($_GET['sSearch_'.$i])."%' ";
		}
	}
	...
	$sQuery2 = "
		SELECT FOUND_ROWS()
	";
	$rResultFilterTotal = DB::query($sQuery2)->fetchColumn();
	...
	$rResultTotal = DB::query($sQuery3)->fetchColumn();
	$iTotal = $rResultTotal;
	...
	foreach ( $rResult as $aRow )
	{
		...
		{	
			...
			{							
				...
				$title2 = "<span style='display:none'>$title</span><a href='../$link$clink ' target='_blank' class='outlink'>$title</a> ";

				if(!$title)
					$row[] = "<em>". Data_not_found ."</em>";
				else 
					$row[] = "$title2";
			}
			else if ( $i == 5 )
