	...
	<div class="box article-editor">								
		...
		<div style="padding:10px 0 0; overflow: hidden;">
			<div class="load-editor">
				...
				<?php
					if(checkLocalhost()) {
						$article = str_replace("media/",FLocal."media/",html_entity_decode($row['article'],ENT_NOQUOTES));			
					}
					echo Form::textarea(
						'text', 
						$article, 
...
<div class="panel-group col-lg-3 box-right article-box" id="accordion">
 <div class="panel box"> 		
	...
	<div id="collapseOne" class="panel-collapse in">
		<table class="data2">
			...
			<tr>
				<td class="row-title" title="Layout" style="width:30%">Layout</td>
				<td>					
					<?php						
						$sql = DB::table(FDBPrefix.'theme_layout')
								->select('id, name')->get();
									
						$qr = [0 => "Follow Parent Category"];
						if($sql)
						foreach($sql as $k=>$v) {
							$qr[$v['id']] = $v['name'];				
						}
			...
			<tr>
				...
				<?php
					$sql = DB::table(FDBPrefix.'article_tags')
						->get();
                  	$qr = [];
                    foreach($sql as $k=>$v) {
                       $qr[$v['name']] = $v['name'];				
 ...
 <div class="panel box"> 		
	<header>
		<a class="accordion-toggle collapsed auto-width-chosen" data-toggle="collapse" data-parent="#accordion" href="#publishing">
  ...
  <div id="publishing" class="panel-collapse collapse">
		<table class="data2">
			...
			<tr>
				...
				<td>		
				 <div id="datetimepicker" class="input-append datetime input-group" style="  width: 160px;">
			...
			<tr>
				...
				<td class="auto-width-chosen">
					
					<?php
						$sql = Database::table(FDBPrefix.'user_group')->where('level >= '.USER_LEVEL)
								->select('level, group_name')->get();
						...
						echo Form::select(
							'level',$qr, $editor_level,
							["class" => "form-control level filter", "name"=>"param12", "style" => "width: 300px !important"]
						);
					?>
				</td>
			</tr>		
