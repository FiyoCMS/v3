...
function option_sub_cats($parent_id,$pre = "__") {
	if($_REQUEST['id']) $eid = "AND id!=$_REQUEST[id]"; else $eid = '';
	$sql = Database::table(FDBPrefix.'article_category')
		->where("parent_id=$parent_id AND id!=$_REQUEST[id]")
		->select('id,parent_id, name')->get();
	$row = [];		
	foreach($sql as $k=>$v) {
		$row[$v['id']] = $pre.'|_ '.$v['name'];							
		$row += option_sub_cat($v['id'],$v['parent_id'], $pre."__");
	}
	return $row;
	
}

function option_sub_cat($parent_id,$pre = "__") {
...
$(document).ready(function(){
	...
	if(type == 'item'){		
  	...
  	$(".popup").click(function() {
		$.ajax({
			url: "?app=article&api=article-menu",
			data: "access",
	...
	<div id="article-parameter" class="in">
		<table>				
			...
			<tr class="category">
				...
				<td>
					<?php						
						$sql = DB::table(FDBPrefix.'article_category')
						...
						foreach($sql as $k=>$v) {
							$row[$v['id']] = $v['name'];							
							$row += option_sub_cats($v['id'],'');
						}
						echo Form::select(
							'now',$row, $aId,
							["class" => "form-control category-select",'id' => 'cat']
						);
					?>
				</td>
			</tr>	
