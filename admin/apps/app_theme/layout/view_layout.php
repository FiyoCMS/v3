...
<form method="post" id="form">
	<div id="app_header">
		<div class="warp_app_header">		
		  ...
		  <div class="app_link">			
			<a class="add btn btn-primary" href="?app=theme&view=layout&act=add"><i class="icon-plus"></i> <?php echo Add; ?></a>
			<button type="submit" class="delete btn btn-danger" title="<?php echo Delete; ?>" value="<?php echo Delete; ?>" name="delete_layout"><i class="icon-trash"></i> &nbsp;<?php echo Delete; ?></button>
		  </div> 
		  <?php printAlert(); ?>		  
		</div>
	</div>
			...
			<tr>								  	
				...
				<th style="width:70% !important; text-align:center">
                                ...
                                <th style="width:10% !important; text-align:right">
                                    <?php echo Theme; ?> <i>(folder name)</i></th>
			</tr>
		</thead>
		<tbody>
			<?php		
			$sql= DB::Table(FDBPrefix.'theme_layout')->get();
			$no=1;
			foreach($sql as $row){
                            if($no == 1) 
				...
				echo "<tr>";
				echo "<td align='center'>$checkbox</td><td>$name &nbsp; <i>(id=$row[id])</i></td><td align='center'>$row[description]</td><td align='center'>$row[theme]</td>";
				echo"</tr>";
				$no++;	
