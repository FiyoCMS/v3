...
function sub_menu($parent_id,$pre,$nos) {
	...
	foreach($sql as $row){
		...
		$name = "<a class='tips' title='".Edit."' data-placement='right' href='?app=menu&act=edit&id=$row[id]'>$row[name]</a>";
		...
		$status ="
				...
				<div class='switch s-icon home'>
					...
					<input type='text' value='$row[id]' data-category='$row[category]' id='id' class='invisible'>
		...
		$default ="<div class='switch s-icon star'>
			...
			<div class='invisible'>
			<input type='text' value='$row[id]'  class='invisible' id='id'>
			<input type='text' value='fp' id='type' class='invisible'>
			</div>
		</div>";	
		...
		$checkbox $name
			...
			<div class='switch-group'>$home$status</div>
				";
		echo "</header></div>";
		sub_menu($row['id'],$pre."","$nos.$no");echo "</li>";
		$no++;	
	}
	if($c) echo "</ol>";
}
