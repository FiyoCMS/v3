...
$(document).ready(function(){	
    ...
    var username  = $("#username").val(); 
		$.ajax({
		 type:"POST",
		 url:"?app=user&api=checkuser",    
		 data: "act=user&username=" + username,
		 success: function(data){                
			if(data==0){
				$("#username").parent().append("<span class='form_ok'>available</span>");		
				$(".form_error").remove();	
			} 
			else if(data==2){
				$("#username").parent().append("<span class='form_error'>Username not valid</span>");		 
				$(".form_ok").remove();	 
			} 
			else {
				
				console.log(data);
				$("#username").parent().append("<span class='form_error'>Username not available</span>");
				$(".form_ok").remove();	 
		   }
		}
		}); 		
	...
	$("#email").change(function(){ 		
		...
		$.ajax({
		 ...
		 success: function(data){              
			if(data==0){
				$("#email").parent().append("<span class='form_ok'>Email&nbsp;available</span>");	
				$(".form_error").remove();	
			} 
			else if(data==2){
				...
				$("#email").parent().append("<span class='form_error'>Email not available</span>");   
				$(".form_ok").remove();	
		   }
		}
		}); 		
				...
				<tr>
					...
					<?php
						echo Form::textarea(
							'bio', 
							'', 
							["class" => "form-control ", "style" => "min-width: 60%", "rows" => "7"]
						);
					?></textarea></td>
				</tr>
			</table>	
