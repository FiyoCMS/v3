...
<form method="post" id="form" enctype="multipart/form-data" target="">
	...
	if(!empty($dfile)) {
		...
		if(file_exists($file)) {
			...
			<label class='cb-default red tips' data-placement='top' title='".Delete_file."'><span><b class='icon-remove-sign'></b></span></label></div>";
			$cfile = "<b><a target='_parent' href='$file' data-placement='top' title='Download File' class='tips'>$dfile</a></b> <small><i>(".format_size(filesize($file))." - ".date("Y/m/d H:i:s",filemtime($file)).")</i></small> $delete";
		...
		<div>
			<table>				
				<tr>
					...
					<td>
					<button class="btn btn-success backup_install" type="submit" name="backup_install" file="<?php echo $nfile; ?>"><?php echo Backup; ?></button></td>
				</tr>		
				<tr>
		...
		<div>
			<table>
				<tr>
					...
					<button class="btn btn-primary btn-grad restore" name="restore">Restore</button></div></td>
				</tr>
			</table>
		</div> 
    </div> 	
