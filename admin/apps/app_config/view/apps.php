...
<table class="data">
	...
	<?php	
	$db = new FQuery(); 
	$sql =	DB::table(FDBPrefix.'apps')->where("name ASC"); 
	$app = null;
	foreach($sql as $app){		
		$file = "../apps/$app[folder]/app_details.php";
		if(file_exists($file))
			include($file);
		echo "<tr>";
					
		if(!isset($app) AND is_array($app)) {
			$app['desc'] = "Error Apps!";
			$app['name'] ="Error Apps!";
		}
		
		if($app['author'] == 'Fiyo CMS' AND (($app['type'] == '0') or ($app['type'] == '2')))
		$action ="
			...
			<div class='switch s-icon uninstall activator' id='$app[folder]'>
				<label class='cb-default red'><span>
				<i class='icon-remove-sign'></i> Uninstall</span></label>				
			</div>";
			
		echo "<td><a class='help' title='$app[name]'>$app[name]</a></td><td>$app[author]</td><td align='right'>$action</td>";
		echo "</tr>";
	}
	?> 
	</tbody>
