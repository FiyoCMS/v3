...
function FQuery($table, $where = null, $output = null, $hide = null, $order = null, $select = null, $limit = null) {
	...
	$sql = Database::table(FDBPrefix."$table")
		->select($select)
		->where($where)
		->orderBy($order)
		->limit($limit)->get();
...
function oneQuery($table,$field,$value,$output = '*') {
	if(DB::tableExists(FDBPrefix.$table))  {
		$value = str_replace("'","",$value);
		$query = Database::table(FDBPrefix.$table)->select($output)->where("$field = '$value'")->get();
		if($query) {
			if($output == '*')
				return $query[0];
			else 
...
function session($var, $strip = true) {
...
function menuInfo($value, $url = null, $id = null, $match = false) {
	if(empty($id)) {
		...
		if(_FINDEX_ !== 'BACK') 
            $isDapur = " AND category != 'adminpanel' AND app != 'sperator'"; 
        else 
            $isDapur = "AND category = 'adminpanel'";

        if(isHomePage()) {
            $c = Database::table(FDBPrefix.'menu')
				->where("home = 1 AND status=1 AND app != 'sperator' $isDapur")
				->select("$value")
				->orderBy("LENGTH(`link`)")->get();
		}
		else if(!$match)
			$c = Database::table(FDBPrefix.'menu')
				->where("link LIKE '%$url%' AND status=1 AND app != 'sperator' $isDapur")
				->select("$value")
				->orderBy("LENGTH(`link`)")->get();
		else
			$c = Database::table(FDBPrefix.'menu')
...
// mengambil informasi menu
function FLayout($value, $layout = null) {
	if(empty($layout)) {
...
function homeInfo($field) {
	$c = Database::table(FDBPrefix.'menu')
		->where('home = 1')->get();
		if(isset($c[0][$field])) return $c[0][$field];
}
...
function app_param($output = null){
	...
	elseif(Input::get($output)) {
		$source = getUrl().'&';
	}
	elseif(Input::request('link')) {	
		//tidak aktif ketika di AdminPanel
		$source = check_permalink('permalink',Input::request('link'),'link').'&';
...
function url($source) {
	$s = getBetweenString("{","}", $source);
	for($i = 0; $i < count($s); $i++){
		$source = str_replace("{{$s[$i]}}", app_param("$s[$i]"), $source);
	}
	return $source;
}
//fungsi membuat permalink
function make_permalink($source, $id = null, $page = null, $like = null, $html = false){
	...
	else if(SEF_URL)
	{
		...
		$db -> connect(); 
		if($like == true)
		$sql = Database::table(FDBPrefix."permalink")->where("link LIKE '%$source%'")->get();
		else
		$sql  = Database::table(FDBPrefix."permalink")->where("link ='$source'")->get();
...
function isHomePage(){	
	...
	if(_FINDEX_ == 'ADMIN' OR  _FINDEX_ == 'BACK') {
		return false;
	}
	else if(FUrl('auto') == $link or empty($link))
		return true;
...
function getLink() {
	...
	$link = str_replace('"',"",$link);
	if(checkLocalhost()) {
		$base = str_replace('localhost','',FUrl());
		$link = str_replace($base,'',$link);
	}
	if(siteConfig('sef_url') AND _FINDEX_ !== 'BACK') {
...
function loadPlugin() {	
	$qr = Database::table(FDBPrefix.'plugin')->where('status=1')->get();	
	foreach($qr as $qr){	
...
//Kalender Indonesia
function kalender($r, $format = null) { 
    if($format)
     $t =  date($format, strtotime($r));
    else
        $t =  date("d F Y", strtotime($r));
...
function alert($type , $text = null, $echo = null, $style = null, $session = null){
	if($style == true or $style == 1 or !empty($style)) {
		...
		if($type == 'error') {
			$type = 'danger error';
			$type2 = 'alert-danger alert-error';

		}
		$alert = "<div class='alert $type $type2' $style>$text</div>";
	}
...
function notice($type , $text, $num = 0) {
	...
	if($type == 'error' AND empty($num)) $num = 2;
...
//fungsi multiple select yang telah terpilih
//$item jika punya child atau isi
//$cat index atau rujukan ID
function multipleDelete($table, $source, $item = null, $reff = null, $except = null, $sub = null) {	
	$del = explode(",",$source);
	if(!isset($except)) $except = null; else $except = $except;
	if(!empty($reff)){ $reff = $fid = $reff; } else{ $reff = 'category'; $fid ='id';}
	if(isset($source))
		foreach($del as $id){
			if(!empty($item)) { 
				if(!empty($except)) 
					$art = Database::table(FDBPrefix.$item)->where("$except AND $reff ='$id'")->get();	
				else
					$art = Database::table(FDBPrefix.$item)->where("$reff ='$id'")->get();
				...
				if(!empty($except)) {
					$art = Database::table(FDBPrefix.$table)->where("$except AND $fid = $id")->get();
					if(count($art)>0) {
						$noempty = 1;					
						break;
					}
				...
				if(!isset($noempty)) {	
					if(!empty($sub)) {
						if(!oneQuery($table,'parent_id',$id))
							$qr = Database::table(FDBPrefix.$table)->delete("$fid='$id'");
						else 						
							$noempty = 1;	
					}
					else {
				...
				if(!empty($except)) { 
					$art = Database::table(FDBPrefix.$table)->where("$except AND $fid ='$id'");		
					if(count($art)>0) {		
						$cantdelete = 1;		
						break;
					}	
...
//fungsi multiple select yang telah terpilih
function multipleDeletes($table, $source, $item = null, $cat = null, $except = null, $sub = null) {  
	$del = explode(",",$source);
	if(!isset($except)) $except = null; else $except = $except;
	...
	if(isset($source))
		foreach($del as $id){
			if(!empty($item)) {
				if(!empty($except)) 
					$art = Database::table("$item")->where("$except AND $cat ='$id'")->get();	
				else
					$art = Database::table("$item")->where("$cat ='$id'")->get();
				...
				if(!empty($except)) { 
					$art = Database::table($table)->where("$except AND $fid = $id")->get();	
					if(count($art)>0) {
						$noempty = 1;					
						break;
					}
				...
				if(!isset($noempty)) {	
					if(!empty($sub)) {
						if(!oneQuery($table,'parent_id',$id))
							$qr = Database::table($table)->delete("$fid='$id'");
						else 						
							$noempty = 1;	
					}
					else {
...
function array_query($post, $select) {	
	$tmps = $select;
	$tmp = [];
	...
	foreach($select  as $k => $v) {
		if(!empty($select[$k])) 
			...
			$v = null;
		$select[$k] = str_replace('{{$value}}',$select[$k],$v); 
	}
	
	return $select;		
}	

function print_messages($array = null, $type = null) {
	static $flag ;
	static $stype ;
	static $sarray ;
	if($type !== 'error') {
...
function page ($code) {
	...
	$template = "<div <div style='vertical-align: middle;text-align: center;width: 97%;height: 50%;position: absolute;top: 45%;margin: 0 auto; color: #666; font-size: 200%'> <b> $code </b> </div>";
	return die($template);
}
class FConfig {
    private function conf()
    {
        $sql = Database::table(FDBPrefix."setting")->select("name, value")->get();	
        $val = null;
        if(!$sql) 
    ...
    public function gets($name = null) {
