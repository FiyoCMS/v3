...
$sql = DB::table(FDBPrefix."article")->where("$filter $level AND status=1")->orderBy($type)->limit($item)->get();
...
foreach($sql as $qr){	
	...
	$date = "<div class='mod-article-date'><div><div class='date'>$day</div><span>$month/$year</span></div></div>";				
	if($type=="hits DESC") {
	$h = $qr['hits'];
	if($h > 900) {
		$h = $h / 1000;
