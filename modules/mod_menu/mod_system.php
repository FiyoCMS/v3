...
function sub_menu($parent_id){
	$qr = DB::table(FDBPrefix."menu")->where("parent_id=$parent_id AND status=1")->orderBy('short ASC')->get(); 
    $sum = count($qr);	
	$no = 1;
	if($sum)
    ...
    foreach($qr as $menu) {
			...
			if ($menu['home']==0){
				...
				elseif ($menu['app']=="link"){
					...
					echo "<li class=\"menu".$menu['class']."$a$post\" style=\"".$menu['style']."\"><a href=\"".FUrl."\">".$menu['name']."$subtitle</a>";
					if(_Mod_SubMenu_ == 1) sub_menu($menu['id']);
					echo "</li>";
			}
			$no++;
