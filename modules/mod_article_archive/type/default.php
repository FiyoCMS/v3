...
if(!$cat) 
	...
	$filter = "date >= $start";
	$archveQuery = DB::table(FDBPrefix."article")->select("*,DATE_FORMAT(date,'%m') as m,DATE_FORMAT(date,'%M') as mo,DATE_FORMAT(date,'%d-%b') as month,DATE_FORMAT(date,'%Y') as y"," $filter AND $catn")->orderBy("DATE DESC")->get();
	$no = $x = 0;
